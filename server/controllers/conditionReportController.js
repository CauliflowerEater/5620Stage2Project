...
const {
  ...
  getConditionReports,
} = require("../services/userServices/conditionReportService");
...
const storeConditionReportController = async (req, res) => {
  ...
  try {
    ...
    await storeConditionReport(userId);
...
const generateConditionReportController = async (req, res) => {
  const { userId } = req.params;
  try {
    const report = await generateConditionReport(userId);
    await storeConditionReport(userId, report);
    res.status(200).json({ message: "Condition report stored successfully" });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
